...
// discord renders this in the reader's own locale and timezone; a formatted date would not
const day = (iso) => {
  const at = new Date(iso).getTime();

  return Number.isFinite(at) ? `<t:${Math.floor(at / 1000)}:D>` : 'no date';
};

// null is no such role; a row with no grant date is one twitch never dated
const holding = (entry) =>
  entry ? `held\n-# ${entry.grantedAt ? day(entry.grantedAt) : 'no date'}` : 'not held';

export const rolesEmbed = ([login, channel], held) => ({
  color: MODDEX_COLOR,
  author: { name: login, url: `https://moddex.tv/user/${login}` },
  description: [
    `**${escape(login)}** ${Object.values(held).some(Boolean) ? 'holds a role in' : 'holds nothing in'} **${escape(channel)}**`,
    `-# ${escape(channel)} on ${links({ login: channel }, 'channel')}`
  ].join('\n'),
  fields: pairs([
    { name: 'Mod', value: holding(held.mod), inline: true },
    { name: 'Vip', value: holding(held.vip), inline: true },
    { name: 'Founder', value: holding(held.founder), inline: true }
  ])
});
...
// which of the two is missing is exactly what a 404 may not say, so it names neither
export const notFoundReply = (logins) =>
  logins.length > 1
    ? `Either **${escape(logins[0])}** or **${escape(logins[1])}** is not in the moddex index.`
    : `**${escape(logins[0])}** is not in the moddex index.`;
